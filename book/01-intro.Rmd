...
This is done using `(library(package_name))`. This is like **launching** an app on your phone: the functionality is only there where the app is launched and remains there until you close the app or restart. For example, when you run `r hl(library(rio))` within a session, the functions in the package referred to by `rio` will be made available for your R session. The next time you start R, you will need to run `r hl(library(rio))` again if you want to access that package.

After installing the`r pkg("esquisse")` package, you can load it for your current R session as follows:
