...
ggplot(data = count_data, 
       mapping = aes(x = issue_category, y = n)) +
  geom_col()
```
...
```{r violin-combos, fig.height = 3, fig.width=10, fig.cap="Violin plots combined with different methods to represent means and medians."}
...
```{r col-plot-bad, message=FALSE, echo = FALSE, fig.cap="Don't plot continuous data with column plots. They are only appropriate for count data."}
tall_col <- ggplot(data = survey_data, 
       mapping = aes(x = issue_category, 
                     y = wait_time,
                     fill = issue_category)) +
  scale_x_discrete(name = "Issue Category") +
...
lm_plot <- ggplot(data = survey_data,
                  mapping = aes(x = wait_time, y = call_time)) +
  geom_point(alpha = 0.2) +
  geom_smooth(method = lm, formula = y~x) +
  ggtitle("method = lm")

loess_plot <- ggplot(data = survey_data,
                     mapping = aes(x = wait_time, y = call_time)) +
  geom_point(alpha = 0.2) +
