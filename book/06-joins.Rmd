...
`r kable(customers, cpation="Demo customers table.")`
...
Let's recreate our `new_customers` dataset but this time, we'll specify that `id` is a character `r glossary("character")` variable.
...
If you try to join this dataset to any of the other datasets where `id` is stored as a `r glossary("numeric")` variable, it will produce the error 
